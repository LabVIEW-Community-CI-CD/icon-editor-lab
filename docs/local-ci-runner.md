...
Docs + coverage knobs:
- `docs_stage.check_links` / `docs_stage.markdownlint` toggle each half of stage 28, while `docs_stage.allow_missing` (exact paths) and `docs_stage.allow_missing_globs` (patterns) extend the baked-in allowlist for still-in-flight files like `ENVIRONMENT.md`.
